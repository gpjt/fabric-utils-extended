...
class ExtendedConnection(Connection):
    ...
    def put_sudo(self, local_path_or_content, remote_path, **kwargs):
        ...
        super().put(local_path_or_content, temp_name, **kwargs)
    ...
    def put_interpolated(self, local_path, remote_path, interpolation, use_sudo=False, **kwargs):
        with open(local_path, "r") as f:
            content = f.read()
        interpolated_content = io.StringIO(content.format(**interpolation))
        return self.put(interpolated_content, remote_path, use_sudo=use_sudo, **kwargs)
